...
serve(async (req) => {
  ...
  try {
    ...
    edgeLogger.info('Found trials ending in ~2 days', { count: day12Trials?.length || 0 });
    ...
    edgeLogger.info('Found trials ending in ~1 day', { count: day13Trials?.length || 0 });
    ...
    for (const trial of day13Trials || []) {
      ...
      if (!hasPaymentMethod) {
        edgeLogger.info('Sending day 13 (URGENT) reminder', { practiceName: profile.name });
