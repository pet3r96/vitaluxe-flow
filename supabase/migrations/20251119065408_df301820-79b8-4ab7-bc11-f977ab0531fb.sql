-- Fix get_rep_earnings RPC to properly handle prescription product checks
-- Replaces direct reference to non-existent op.is_rx_required with proper join logic
  ...
  SELECT
    ...
    CASE 
      WHEN EXISTS (
        ...
        WHERE ol.order_id = op.order_id AND prod.requires_prescription = true
      ) THEN 0
