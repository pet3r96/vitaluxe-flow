-- Fix staff access to see all products like admin
...
begin
  -- Admins AND STAFF see all active products (bypass filtering)
  is_admin := EXISTS (
  ...
  if my_topline is null then
    if has_role(p_effective_user_id, 'doctor'::app_role) or has_role(p_effective_user_id, 'provider'::app_role) then
