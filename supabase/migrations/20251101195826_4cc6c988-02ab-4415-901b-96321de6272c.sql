-- Update get_provider_documents to include patient-shared documents
...
BEGIN
  RETURN QUERY
  -- Provider documents (existing logic)
  SELECT 
    pd.id,
    pd.practice_id,
    pd.document_name,
  ...
  SELECT 
    ...
    pat_doc.notes,
    pat_doc.status,
    false as is_internal, -- Patient docs are never internal
