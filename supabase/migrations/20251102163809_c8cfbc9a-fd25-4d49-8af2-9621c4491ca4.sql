-- Create helper function to manually recalculate order line profits
CREATE OR REPLACE FUNCTION recalculate_order_profits_for_practice(practice_user_id UUID)
RETURNS TABLE (
  order_line_id UUID,
  topline_rep_id UUID,
  downline_rep_id UUID,
  topline_profit DECIMAL,
  downline_profit DECIMAL
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FROM profiles WHERE id = practice_user_id;
  
  -- Handle rep identification
  IF v_topline_user_id IS NOT NULL THEN
    SELECT r.id, r.assigned_topline_id 
    INTO v_downline_rep_id, v_topline_rep_id
    FROM reps r
    WHERE r.user_id = v_topline_user_id AND r.role = 'downline'
    LIMIT 1;
    
    IF v_downline_rep_id IS NULL THEN
      SELECT id INTO v_topline_rep_id
      FROM reps WHERE user_id = v_topline_user_id AND role = 'topline'
      LIMIT 1;
    END IF;
  END IF;
  ...
  FOR order_line IN 
    ...
    WHERE o.doctor_id = practice_user_id
  LOOP
    -- Get product pricing
    ...
    FROM products WHERE id = order_line.product_id;
    ...
    -- Delete old profit record if exists
    DELETE FROM order_profits WHERE order_line_id = order_line.id;
    
    -- Insert new profit record
    INSERT INTO order_profits (
      order_id, order_line_id, topline_id, downline_id,
      base_price, topline_price, downline_price, practice_price,
      topline_profit, downline_profit, admin_profit,
      quantity, is_rx_required
    ) VALUES (
      order_line.order_id, order_line.id, v_topline_rep_id, v_downline_rep_id,
      v_base_price, v_topline_price, v_downline_price, v_practice_price,
      v_topline_profit, v_downline_profit, v_admin_profit,
      order_line.quantity, v_is_rx_required
    );
    
    -- Return results
    order_line_id := order_line.id;
    topline_rep_id := v_topline_rep_id;
    downline_rep_id := v_downline_rep_id;
    topline_profit := v_topline_profit;
    downline_profit := v_downline_profit;
    RETURN NEXT;
  END LOOP;
END;
$$;