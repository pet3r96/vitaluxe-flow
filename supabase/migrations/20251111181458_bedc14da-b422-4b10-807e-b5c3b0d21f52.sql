-- Fix notify_patient_of_practice_message trigger to use profiles table instead of practices
-- AND ensure handleNotifications is called to send email/SMS/in-app notifications
...
BEGIN
  ...
  -- FIX: Use profiles table instead of practices
  SELECT name INTO practice_name
  FROM profiles
  WHERE id = NEW.practice_id;
  
  -- Send notification via handleNotifications to ensure email/SMS/in-app delivery
