-- Fix get_patient_unified_documents to show documents with 'uploaded' status
-- Remove the status filter that was preventing newly assigned documents from appearing
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(pd.is_hidden, false) as is_hidden
  FROM public.patient_documents pd
  JOIN public.patient_accounts pa ON pd.patient_id = pa.id
  WHERE pd.patient_id = p_patient_id
    AND COALESCE(pd.is_hidden, false) = false

  UNION ALL

  -- Provider documents assigned to this patient
  -- FIXED: Removed status filter to allow 'uploaded' status documents
  SELECT
