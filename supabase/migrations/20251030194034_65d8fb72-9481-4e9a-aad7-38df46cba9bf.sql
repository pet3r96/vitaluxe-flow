-- Update get_practice_hours_with_defaults to handle both 0-6 and 1-7 day numbering
...
AS $$
  -- Try to get configured hours first (handle both 0=Sunday and 7=Sunday)
  SELECT 
    pch.start_time::TIME,
    pch.end_time::TIME,
    pch.is_closed
  ...
  WHERE pch.practice_id = p_practice_id
    AND pch.day_of_week IN (p_day_of_week, CASE WHEN p_day_of_week = 0 THEN 7 ELSE p_day_of_week END)
  ...
  WHERE NOT EXISTS (
    ...
    AND day_of_week IN (p_day_of_week, CASE WHEN p_day_of_week = 0 THEN 7 ELSE p_day_of_week END)
  )
  
  LIMIT 1;
$$;

-- Update get_patient_appointments_with_details to include provider display name
...
BEGIN
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'provider', CASE 
        WHEN prov.id IS NOT NULL THEN
          jsonb_build_object(
            'display_name', COALESCE(
              provider_prof.name,
              NULLIF(TRIM(COALESCE(provider_prof.first_name, '') || ' ' || COALESCE(provider_prof.last_name, '')), ''),
              provider_prof.email,
              'Provider'
            ),
