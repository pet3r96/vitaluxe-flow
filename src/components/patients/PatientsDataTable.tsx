...
export const PatientsDataTable = () => {
  ...
  // Use centralized hook for patient data (bypasses RLS via edge function)
  const { data: patients, isLoading, refetch } = usePatients(effectivePracticeId);

  // Log patient fetch results
  logger.info('Patients fetched via hook', { count: patients?.length || 0 });
