...
export const ProviderVirtualWaitingRoom = ({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data: sessionsData,
        error: sessionsError
      } = await supabase.from('video_sessions').select('*, patient_accounts!video_sessions_patient_id_fkey(id, first_name, last_name, name)').eq('practice_id', practiceId).gte('scheduled_start_time', yesterday.toISOString()).lt('scheduled_start_time', dayAfterTomorrow.toISOString()).in('status', ['created', 'scheduled', 'waiting', 'active']).order('scheduled_start_time', {
        ascending: true
      });
      ...
      } = await supabase.from('patient_appointments').select('id, patient_id, provider_id, start_time, status').eq('practice_id', practiceId).eq('visit_type', 'video').not('status', 'in', '("cancelled","completed")') // Exclude only terminal statuses
      .gte('start_time', yesterday.toISOString()).lt('start_time', dayAfterTomorrow.toISOString()).order('start_time', {
        ascending: true
      });
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('patient_accounts').select('id, first_name, last_name, email, primary_provider_id, provider_id').eq('practice_id', practiceId).order('last_name');
      ...
      queryClient.prefetchQuery({
        ...
        queryFn: async () => {
          const {
            data,
            error
          } = await supabase.from('patient_accounts').select('id, first_name, last_name, email').eq('practice_id', practiceId).order('last_name');
          if (error) throw error;
          return data || [];
        }
      });
  ...
  } = useProvidersAndStaff(practiceId);
  ...
  const filteredProviders = useMemo(() => {
    ...
    const assignedProviderIds = [selectedPatient.primary_provider_id, selectedPatient.provider_id].filter(Boolean);
  ...
  const handlePrepareSession = async (sessionId: string) => {
    ...
    try {
      ...
      const {
        data: ensureData,
        error: ensureError
      } = await supabase.functions.invoke('ensure-video-session', {
        body: {
          appointmentId
        }
      });
    ...
    try {
      ...
      const waitForVideoSessionEnd = async () => {
        for (let i = 0; i < 10; i++) {
          ...
          const {
            data
          } = await supabase.from('video_sessions').select('id, status').eq('appointment_id', appointmentId).maybeSingle();
  ...
  const handleCompleteAppointment = async (appointmentId: string) => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.functions.invoke('complete-video-appointment', {
        body: {
          appointmentId
        }
      });
      ...
      const waitForCompletion = async () => {
        for (let i = 0; i < 10; i++) {
          const {
            data
          } = await supabase.from('patient_appointments').select('id, status').eq('id', appointmentId).maybeSingle();
  ...
  const handleCreateInstantSession = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('create-instant-video-session', {
        body: {
      ...
      const {
        error: startError
      } = await supabase.functions.invoke('start-video-session', {
        body: {
  ...
  if (!videoSessions || videoSessions.length === 0) {
    return <>
        <Card>
          <CardHeader>
            <div className="flex items-start justify-between">
              <CardTitle className="flex items-center gap-2">Upcoming Video Sessions    <Video className="h-5 w-5 text-primary" />
                Virtual Waiting Room
              </CardTitle>

              <div className="flex flex-col gap-2 relative z-10">
                <Button onClick={() => setShowScheduleDialog(true)} variant="secondary" className="gap-2 w-full cursor-pointer">
                  <Calendar className="h-4 w-4" />
                  Schedule Video Appointment
                </Button>
                
                <Button onClick={() => setShowCreateDialog(true)} className="gap-2 w-full cursor-pointer">
        ...
        <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Select value={selectedPatientId} onValueChange={setSelectedPatientId}>
                  ...
                  <SelectContent>
                    {patientsLoading ? <SelectItem value="loading" disabled>
                        <div className="flex items-center gap-2">
                          <Loader2 className="h-3 w-3 animate-spin" />
                          Loading patients...
                        </div>
                      </SelectItem> : patientsError ? <SelectItem value="error" disabled>
                        Failed to load patients. Try again.
                      </SelectItem> : patients && patients.length > 0 ? patients.map((patient: any) => <SelectItem key={patient.id} value={patient.id}>
                          {getPatientDisplay(patient)}
                        </SelectItem>) : <SelectItem value="no-patients-available" disabled>
              ...
              <div className="space-y-2">
                ...
                <Select value={selectedProviderId} onValueChange={setSelectedProviderId}>
                  ...
                  <SelectContent>
                    {!selectedPatientId ? <SelectItem value="no-patient-selected" disabled>
                        Please select a patient first
                      </SelectItem> : filteredProviders && filteredProviders.length > 0 ? filteredProviders.map((provider: any) => {
                    const displayName = getProviderDisplayName(provider);
                    return <SelectItem key={provider.id} value={provider.id}>
                            <div className="flex items-center justify-between w-full gap-2">
                              <span>{displayName}</span>
                              {provider.type && <Badge variant={provider.type === 'provider' ? 'default' : 'secondary'} className="ml-2 text-[10px] px-1.5 py-0">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={handleCreateInstantSession} disabled={!selectedPatientId || !selectedProviderId || creatingSession} className="gap-2">
                {creatingSession ? <>
        ...
        <CreateAppointmentDialog open={showScheduleDialog} onOpenChange={setShowScheduleDialog} practiceId={practiceId} providers={allProviders?.map(p => ({
        id: p.id,
        full_name: p.full_name || getProviderDisplayName(p),
        first_name: p.first_name || '',
        last_name: p.last_name || '',
        type: p.type
      })) || []} rooms={[]} defaultVisitType="video" />
      </>;
  }
  return <Card>
      <CardHeader>
          ...
          <CardTitle className="flex items-center gap-2">
            ...
            <DialogContent>
              ...
              <div className="flex gap-2 justify-end">
                ...
                <Button onClick={() => setShowScheduleDialog(true)} variant="secondary" className="gap-2 w-full">
                  <Calendar className="h-4 w-4" />
                  Schedule Video Appointment
                </Button>
          </div>
      ...
      <CardContent className="space-y-3">
        ...
        const patientAccount = Array.isArray(session.patient_accounts) ? session.patient_accounts[0] : session.patient_accounts;
        const fullName = patientAccount ? `${patientAccount.first_name || ''} ${patientAccount.last_name || ''}`.trim() : '';
        const patientName = patientAccount ? patientAccount.name || fullName || 'Patient' : 'Patient';
        const appointmentTime = format(new Date(session.scheduled_start_time), 'h:mm a');
        const isPatientWaiting = session.patient_joined_at !== null && session.provider_joined_at === null;
        return <div key={session.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-accent/50 transition-colors">
              <div className="flex items-center gap-4">
                <div className="relative">
                  <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center">
                    <User className="h-5 w-5 text-primary" />
                  </div>
                  {isPatientWaiting && <Circle className="h-3 w-3 fill-green-500 text-green-500 absolute -top-1 -right-1 animate-pulse" />}
                </div>
                
                <div>
                  <div className="font-medium flex items-center gap-2">
                    {patientName}
                    {isSyntheticSession(session.id) && <span className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded">
                        Initializing...
                      </span>}
                  </div>
                  <div className="text-sm text-muted-foreground flex items-center gap-2">
                    ...
                    {isPatientWaiting && <span className="text-green-600 dark:text-green-400 font-medium">
                        • Patient waiting
                      </span>}
                  </div>
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {isSyntheticSession(session.id) && differenceInMinutes(new Date(session.scheduled_start_time), new Date()) >= 10 && <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button variant="outline" size="sm" onClick={() => handlePrepareSession(session.id)} disabled={preparingSession === session.id} className="gap-2 border-primary/50 text-primary hover:bg-primary/10">
                          {preparingSession === session.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <Video className="h-3 w-3" />}
                          Prepare Now
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent>
                ...
                <Button variant="outline" size="sm" onClick={() => handleGenerateGuestLink(session.id)} disabled={generatingLink === session.id} className="gap-2" title="Generate guest access link">
                  {generatingLink === session.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <Link2 className="h-3 w-3" />}
                  Guest Link
                </Button>

                {session.status === 'scheduled' && <>
                    <Button onClick={() => handleStartSession(session.id)} size="sm" className="gap-2" disabled={startingSession === session.id} title="Start the video session">
                      {startingSession === session.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Video className="h-4 w-4" />}
                      Start Session
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => {
                setSessionToCancel(session.appointment_id);
                setShowCancelDialog(true);
              }} disabled={cancellingSession === session.appointment_id} className="text-destructive hover:text-destructive hover:bg-destructive/10" title="Cancel appointment">
                      {cancellingSession === session.appointment_id ? <Loader2 className="h-4 w-4 animate-spin" /> : <X className="h-4 w-4" />}
                    </Button>
                  </>}

                {session.status === 'waiting' && <>
                    <Button variant="default" size="sm" className="gap-2 animate-pulse" onClick={() => handleJoinSession(session.id)}>
                      <Video className="h-4 w-4" />
                      Join Now
                    </Button>
                    {!isSyntheticSession(session.id) && <Button variant="outline" size="sm" className="gap-2 border-green-500 text-green-700 hover:bg-green-50 dark:text-green-400 dark:hover:bg-green-950" onClick={() => {
                setAppointmentToComplete(session.appointment_id);
                setShowCompleteDialog(true);
              }} disabled={completingAppointment === session.appointment_id} title="Mark appointment as completed">
                        {completingAppointment === session.appointment_id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
                        Complete
                      </Button>}
                    <Button variant="ghost" size="sm" onClick={() => {
                setSessionToCancel(session.appointment_id);
                ...
                {session.status === 'active' && !isSyntheticSession(session.id) && <>
                    <Button variant="outline" size="sm" className="gap-2 border-green-500 text-green-700 hover:bg-green-50 dark:text-green-400 dark:hover:bg-green-950" onClick={() => {
                setAppointmentToComplete(session.appointment_id);
                setShowCompleteDialog(true);
              }} disabled={completingAppointment === session.appointment_id} title="Mark appointment as completed">
                      {completingAppointment === session.appointment_id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
                    ...
                    <Button variant="default" size="sm" className="gap-2" onClick={() => handleJoinSession(session.id)}>
                      <Video className="h-4 w-4" />
                      Rejoin
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => {
      ...
      {/* Guest Link Dialog */}
      {guestLinkData && <VideoGuestLinkDialog open={showGuestLinkDialog} onOpenChange={setShowGuestLinkDialog} guestUrl={guestLinkData.url} expiresAt={guestLinkData.expiresAt} />}
      ...
      <AlertDialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => sessionToCancel && handleCancelAppointment(sessionToCancel)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
              Cancel Appointment
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <AlertDialog open={showCompleteDialog} onOpenChange={setShowCompleteDialog}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => appointmentToComplete && handleCompleteAppointment(appointmentToComplete)} className="bg-green-600 text-white hover:bg-green-700">
