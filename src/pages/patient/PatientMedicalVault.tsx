...
export default function PatientMedicalVault() {
  ...
  const { data: auditLogs = [], isLoading: isLoadingAuditLogs } = useAuditLogs(patientAccount?.id);
  ...
  const activeAllergies = (allergies || []).filter((a: any) => a.is_active && !a.nka);
  const activeMedications = (medications || []).filter((m: any) => m.is_active);
  const activeConditions = (conditions || []).filter((c: any) => c.is_active);
  const hasNKA = (allergies || []).some((a: any) => a.nka);
